...
from chatting.chattingui import *
class Qtray(QtCore.QObject):#我没有直接把这个托盘化模块放在window下，我放在Chattingfrontend
    ...
    def __init__(self,username,state):
        ...
        self.icon = QtGui.QIcon("./beautify/systempic/ico.ico")
        self.hasmessage = QtGui.QIcon("./beautify/systempic/hasmessage.ico")
        self.invisibleicon = QtGui.QIcon("./beautify/systempic/invisible.ico")
    ...
    def iconClicked(self,signal):#点击icon事件
        if self.mainshow and (signal == 2 ):
            self.hide.emit()
            self.mainshow = False
        elif not self.mainshow and signal == 2:
            self.reOpen()
...
class Chattingfrontend(QtCore.QObject):
    ...
    def chooseFile(self):#选择文件
        filepath,filetype = QtWidgets.QFileDialog.getOpenFileNames(self.window,"选择上传文件（可多选）","D:/","All Files (*)")
        try:
